...
// No corePath here, deliberately. On the Node path tesseract.js ignores the
// option entirely — its worker script feature-detects relaxed SIMD, then SIMD,
// and `require`s the matching build by name. Passing one looked like it pinned
// the core to a single small file; it did not, and the mismatch is what broke
// this route in production: the deployment traced the pinned build while the
// running function resolved a relaxed-SIMD one that was never shipped with it,
// so every request failed at worker startup. Local runs were fine throughout —
// a full node_modules has all of them.
//
// Which builds have to ship is therefore decided by that feature detection, not
// by us, and next.config.ts traces the set it can choose from. Keep the two in
// step: narrowing the trace to save bundle size only works if it still covers
// every build the runtime might resolve.
