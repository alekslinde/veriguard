...
describe("POST /api/check region resolution", () => {
  ...
  it("uses the base-only pack for a known-but-uncovered country", async () => {
    // "DE" until the DE pack shipped, then "FR" until item 4 step 5's wave 2
    // shipped a French pack too; kept as a country with no national layer
    ...
    const res = await POST(check({ content: SCAM }, { "x-vercel-ip-country": "PT" }));
    const json = await res.json();
